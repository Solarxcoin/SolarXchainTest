...
module.exports = function sendCoin({ ipcRenderer, apis }) {
 ...
 // Check if sender and receiver addresses are the same
    if (address === receiver) {
      ipcRenderer.send(IPC_EVENT_OPEN_MESSAGE_BOX, {
        message: 'Sender and receiver addresses cannot be the same.',
        messageType: 'info'
      });
      return;
    }
    ...
    if (transactionValue < Math.pow(10, -NUMBER_OF_DECIMAL_PLACES)) {
      ipcRenderer.send(IPC_EVENT_OPEN_MESSAGE_BOX, {
        message: `Coin to send should be at least ${Math.pow(10, -NUMBER_OF_DECIMAL_PLACES)} SOLX`,
        messageType: 'error'
      });
      return;
    }
